...
{
    $con=getConnection();
    $sql = "SELECT * FROM upay where id={$id}";
    $result = mysqli_query($con, $sql);
    $data = mysqli_fetch_assoc($result);
    return $data;
}
...
{
    $con=getConnection();
    $sql = "UPDATE upay SET username='{$upaydata['username']}', orderid='{$upaydata['orderid']}', paym='{$upaydata['paym']}', status='{$upaydata['status']}', amount='{$upaydata['amount']}' where id={$upaydata['id']}";
    if (mysqli_query($con, $sql)) {
        return true;
    } else {
        return false;
...
{
    $con = getConnection();
    $sql = "delete from upay where id={$id}";
    if (mysqli_query($con, $sql)) {
        return true;
    } else {
        return false;
